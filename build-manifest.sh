...
if [ "$TOKEN" != "null" ]; then
    ...
    if [ "$RELEASE" == "TEST" ]; then
        ...
        docker manifest create ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION} \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-aarch64 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-armhf \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-armv7 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-ppc64le \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-x86 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}-x86_64

        docker manifest push ${DOCKER_IMAGE}:${ALPINE_VERSION}-${PHP_VERSION}
    else
        echo "Push Manifest for $RELEASE"
        ...
        docker manifest create ${DOCKER_IMAGE}:${PHP_VERSION} \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-aarch64 \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-armhf \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-armv7 \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-ppc64le \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-x86 \
            --amend ${DOCKER_IMAGE}:${PHP_VERSION}-x86_64

        docker manifest push ${DOCKER_IMAGE}:${PHP_VERSION}
    fi
else 
